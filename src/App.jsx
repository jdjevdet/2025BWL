...
const EventEditorCard = ({ 
    ...
    const handleImageUpload = (e) => {
        ...
        if (file) {
            ...
            reader.onloadend = () => {
                setLocalData(prev => ({ ...prev, bannerImage: reader.result }));
            };
...
const FantasyWrestlingApp = () => {
  ...
  const [events, setEvents] = useState(() => {
    try {
      const savedEvents = localStorage.getItem('fantasyWrestlingEvents');
      return savedEvents ? JSON.parse(savedEvents) : [
        {
          id: 1, name: 'BACKLASH', date: 'Saturday, May 3', status: 'completed', bannerImage: null, submittedPlayers: [],
          matches: [
            { id: 1, title: 'World Championship', options: ['Cody Rhodes', 'Seth Rollins'], winner: 'Cody Rhodes' },
            { id: 2, title: 'Tag Team Championship', options: ['The Usos', 'New Day'], winner: 'The Usos' }
          ]
        },
        {
          id: 2, name: 'MONEY IN THE BANK', date: 'Saturday, June 7', status: 'open', bannerImage: null, submittedPlayers: [],
          matches: [
            { id: 1, title: 'Mens MITB Ladder Match', options: ['Drew McIntyre', 'LA Knight', 'Shinsuke Nakamura', 'Ricochet'] },
            { id: 2, title: 'Womens MITB Ladder Match', options: ['Becky Lynch', 'Bayley', 'IYO SKY', 'Tiffany Stratton'] }
          ]
        },
        { id: 3, name: 'NIGHT OF CHAMPIONS', date: 'Sunday, June 28, 2025', status: 'upcoming', bannerImage: null, matches: [], submittedPlayers: [] }
      ];
    } catch (error) {
  ...
  const [players, setPlayers] = useState(() => {
    try {
      const savedPlayers = localStorage.getItem('fantasyWrestlingPlayers');
      return savedPlayers ? JSON.parse(savedPlayers) : [
        { id: 1, name: 'Johnny', picks: {} }, { id: 2, name: 'Jordan', picks: {} },
        { id: 3, name: 'Jaydan', picks: {} }, { id: 4, name: 'Matthew', picks: {} },
        { id: 5, name: 'Erik', picks: {} }, { id: 6, name: 'Albert', picks: {} },
        { id: 7, name: 'Derick', picks: {} }, { id: 8, name: 'George', picks: {} },
        { id: 9, name: 'Marcus', picks: {} }, { id: 10, name: 'Rob', picks: {} },
      ];
    } catch (error) {
        console.error("Could not parse players from localStorage", error);
        return [];
    }
  });
  ...
  const [hallOfFameEntries, setHallOfFameEntries] = useState(() => {
    try {
      const savedHallOfFame = localStorage.getItem('fantasyWrestlingHallOfFame');
      return savedHallOfFame ? JSON.parse(savedHallOfFame) : [];
    } catch (error) {
      console.error("Could not parse Hall of Fame entries from localStorage", error);
      return [];
    }
  });
  ...
  useEffect(() => {
    localStorage.setItem('fantasyWrestlingEvents', JSON.stringify(events));
  }, [events]);

  useEffect(() => {
    localStorage.setItem('fantasyWrestlingPlayers', JSON.stringify(players));
  }, [players]);

  useEffect(() => {
    localStorage.setItem('fantasyWrestlingHallOfFame', JSON.stringify(hallOfFameEntries));
  }, [hallOfFameEntries]);
  ...
  const handlePicksSubmitted = (eventId, playerName) => {
    const eventIndex = events.findIndex(e => e.id === eventId);
    if (eventIndex !== -1) {
      const submittedPlayers = events[eventIndex].submittedPlayers || [];
      if (!submittedPlayers.includes(playerName)) {
        const updatedEvent = {
          ...events[eventIndex],
          submittedPlayers: [...submittedPlayers, playerName]
        };
  ...
  const addMatch = (eventId, newMatch) => {
    if (newMatch.title && newMatch.options.filter(o => o).length >= 2) {
      ...
      const updatedEvent = { ...event, matches: updatedMatches };
  ...
  const submitPick = (eventId, matchId, pick) => {
    if (!currentUser) {
    ...
    if (player) {
      ...
      setPlayers(players.map(p => 
        p.name === currentUser ? { ...p, picks: updatedPicks } : p
      ));
    }
  };
  ...
  const addHallOfFameEntry = (newEntry) => {
    const entryWithId = { ...newEntry, id: Date.now() };
    setHallOfFameEntries([...hallOfFameEntries, entryWithId]);
  };

  const updateHallOfFameEntry = (id, updates) => {
    setHallOfFameEntries(hallOfFameEntries.map(entry => 
  ...
  const FloatingParticles = React.memo(() => (
    <div className="fixed inset-0 overflow-hidden pointer-events-none z-0">
      {particles.map(p => (
        <div
          key={p.id}
          className="absolute rounded-full bg-gray-800" // No longer transparent by default
          style={{
            left: `${p.x}%`,
            top: `${p.y}%`,
            width: `${p.size}px`,
      ...
      <style>{`
        ${particles.map(p => `
            @keyframes drift-${p.id} {
                ...
                25% {
                    transform: translate3d(${p.initialDirectionX * 50}px, ${p.initialDirectionY * 50}px, 0) scale(1.5); /* Larger movement and scale */
                    opacity: 0.4;
                }
                50% {
                    transform: translate3d(${-p.initialDirectionX * 40}px, ${-p.initialDirectionY * 40}px, 0) scale(1.8); /* Even larger scale */
                    opacity: 1; /* Fully opaque at peak */
                }
                75% {
                    transform: translate3d(${p.initialDirectionX * 30}px, ${p.initialDirectionY * 30}px, 0) scale(1.2);
                    opacity: 0.6;
                }
                100% {
                    transform: translate3d(0, 0, 0) scale(1);
  ...
  const MakePicksView = () => {
    ...
    const handleLockInPicks = () => {
        ...
        if (selectedEvent.matches.length > 0 && playerPicksForEvent.length < selectedEvent.matches.length) {
  ...
  const PlayerManagement = ({ isMinimized, onToggleMinimize }) => {
    ...
    const handleAddPlayer = () => {
      if (localPlayerName.trim()) {
        const newPlayer = {
          id: players.length > 0 ? Math.max(...players.map(p => p.id)) + 1 : Date.now(),
  ...
  const HallOfFameManagement = ({ 
    ...
    const handleImageUpload = (e) => {
        ...
        if (file) {
            ...
            reader.onloadend = () => {
                if (editingEntryId) {
                    setLocalEditingData(prev => ({ ...prev, image: reader.result }));
                } else {
    ...
    const handleAddOrUpdate = () => {
        if (editingEntryId) {
            ...
            if (newEntry.title && newEntry.image) {
                onAddEntry(newEntry);
                setNewEntry({ title: '', image: null, description: '' });
            } else {
          ...
          <div className="mb-6 bg-black p-4 rounded-lg border border-gray-800">
            ...
            <div className="border-2 border-dashed border-gray-700 rounded-lg p-4 hover:border-white transition-all duration-300 mb-4">
              <label className="cursor-pointer block text-center">
                ...
                />
                <div className="text-gray-400 hover:text-white transition-colors">
                  {(editingEntryId ? localEditingData?.image : newEntry.image) ? (
                    <div>
                      <img src={editingEntryId ? localEditingData.image : newEntry.image} alt="Preview" className="w-full h-32 object-cover rounded mb-2" />
              ...
              entries.map((entry, index) => (
                ...
                >
                  <div className="flex items-center space-x-3">
                    {entry.image && <img src={entry.image} alt={entry.title} className="w-12 h-12 object-cover rounded-full border border-gray-700" />}
                    <div>
                     ...
                     <div className="mb-12 bg-gradient-to-br from-gray-900 to-black rounded-2xl p-8 shadow-2xl border-2 border-yellow-500/50 relative overflow-hidden group animate-fadeInUp">
                         ...
                         <div className="flex flex-col md:flex-row items-center gap-8">
                             <div className="md:w-1/2 relative">
                                 <img src={latestWinner.image} alt={latestWinner.title} className="rounded-xl shadow-2xl w-full h-auto object-cover border-4 border-yellow-400"/>
