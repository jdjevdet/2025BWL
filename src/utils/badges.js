...
/* ══════════════════════════════════════════════
   NEW BADGE TRACKING (localStorage-based)
   ══════════════════════════════════════════════ */

const STORAGE_KEY = 'bwl-badge-timestamps';

function loadBadgeTimestamps() {
  try {
    return JSON.parse(localStorage.getItem(STORAGE_KEY) || '{}');
  } catch { return {}; }
}

function saveBadgeTimestampsLocal(data) {
  try { localStorage.setItem(STORAGE_KEY, JSON.stringify(data)); } catch {}
}

/**
 * Call this when rendering a player's badges. It compares the current badge
 * list against what's stored and timestamps any new additions.
 * First call for a player: all badges get timestamp 0 (never shows NEW).
 * Subsequent calls: new badges get Date.now() (shows NEW for 12 hours).
 */
export function syncBadgeTimestamps(playerName, badgeIds) {
  const all = loadBadgeTimestamps();
  const stored = all[playerName] || null;
  const isFirstSync = stored === null;

  const updated = { ...(stored || {}) };
  let changed = false;
  badgeIds.forEach(id => {
    if (updated[id] === undefined) {
      updated[id] = isFirstSync ? 0 : Date.now();
      changed = true;
    }
  });
  if (changed) {
    all[playerName] = updated;
    saveBadgeTimestampsLocal(all);
  }
}

/** Check if a badge should show the NEW tag (earned within last 12 hours) */
export function isBadgeNew(playerName, badgeId) {
  const all = loadBadgeTimestamps();
  const ts = all[playerName]?.[badgeId];
  if (!ts) return false; // 0 or undefined = not new
